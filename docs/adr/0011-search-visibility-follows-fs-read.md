...
- ~~**swap 窓では検索が fs:read より厳しくなる**~~ → **[#418](https://github.com/ymnao/scripta/issues/418) で解消**。本 ADR の時点では `fs:read`（`readFileImpl`）が canonical を plain open していたため、末端を swap された瞬間に「検索結果から落ちるが fs:read は開ける」状態になり得た（落とす側に倒れるので境界は破れない非対称）。#418 で `fs:read` / `fs:read-base64` / `fs:write` も O_NOFOLLOW 付き fd I/O に揃えたため、末端 swap に対する挙動は scan と一致する（win32 を除く。#451）
