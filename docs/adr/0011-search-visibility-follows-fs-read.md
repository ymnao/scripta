...
**解決先の read も `readFileUtf8NoFollow`（O_NOFOLLOW fd）で行う**。canonical の末端は realpath 済みで非 symlink なので正常系では発火せず、発火する = 検査（T1）から read（T2）の間に末端を symlink へ差し替えられた瞬間であり、読める実体が検査した実体と違う以上 skip が正しい。これは分岐 1 が #412 で採った判断を alias 側にも揃えたもの。結果として **scan の全 read が「検査した対象そのものから読む」に統一**され、plain `fsp.readFile` を使う経路は無くなった。
...
errno は**見分けない**。ELOOP 以外の失敗（EACCES / ENOENT 等）も同じ fallback に倒れ、解決先が null なら skip、非 null ならその canonical を O_NOFOLLOW で開き直す。**境界判定そのものは errno に依存しない**（symlink かどうかは realpath が答える）。transient な失敗（EMFILE 等）は 2 回目の open も失敗して skip に倒れるが、これは「読めなかった file は結果に出ない」という既存契約と同じ挙動で、検査していない内容が結果に混ざる方向へはズレない。プラットフォーム差（Linux は ELOOP、BSD 系は EMLINK を返し得る）に実装が依存しない利点もある。
...
- ~~**Windows ではゲート未評価経路の境界が効かない**~~ → **[#451](https://github.com/ymnao/scripta/issues/451) で解消**。本 ADR の時点では `fs.constants.O_NOFOLLOW` が無く flag が 0 に落ちるため、分岐 3（index 未提供 / index 無効 / 既に index 済みで valid）では末端 symlink が検出されず、内容がそのまま結果に出た（分岐 2 は realpath ベースなので Windows でも効いており、index の状態によって結果集合が変わる非一貫性になっていた）。#451 で **`O_NOFOLLOW` が使えない platform だけ open の前に `lstat` を挟んで同じ拒否をエミュレート**するようにし（`utils/open-nofollow.ts` の `rejectEndSymlinkWhenEmulated`）、分岐 3 も全 platform で末端 symlink を検出するようになった。**受容していた範囲より影響が広かった**点も併せて記録する: 分岐 3 の open 成功は `readFromVerifiedFd` を立てる = **L2 admission の条件**でもあるため、Windows では外部内容が検索結果に出るだけでなく L2 ContentCache にも載っていた（本節が受容として記述していたのは結果集合の非一貫性までで、L2 admission の不変条件が platform で崩れることは書かれていなかった）。**残る差はエミュレーションの非原子性**: `lstat`（T1）と open（T2）は別 syscall なので、その間に末端を差し替える race は閉じない（POSIX は同じ拒否を open と原子的に行う）。Node が Windows の `FILE_FLAG_OPEN_REPARSE_POINT` 相当を expose していないため埋められず、受容の posture は #412 と同じ（Windows の symlink 作成は既定で管理者特権または開発者モードを要するため、攻撃前提が成立しにくい）。なお **win32 の fs semantics は [#500](https://github.com/ymnao/scripta/issues/500) で実測に置き換えた**（`electron/main/utils/win32-fs-semantics.test.ts` を `windows-latest` の `win32-fs-probe` job で回す。required check ではない）。実測で確認できたのは `O_NOFOLLOW` が undefined であること / `lstat` が file symlink を `isSymbolicLink()=true` で報告すること / plain open が follow すること / directory junction が `readdir` で `isDirectory()=false` になり walk に混入しないこと / エミュレーションの ELOOP 拒否が実機で効くこと。**唯一 POSIX と割れたのは `O_EXCL` の dangling symlink**（win32 は follow して解決先に file を作る → [#504](https://github.com/ymnao/scripta/issues/504)）で、これは本 ADR の分岐 3 ではなく **`writeFileAtomicNoFollow` の tmp 作成と create 系 3 経路（`fs:write-new` / `fs:create-file` / `fs:create-directory`）**に効く。#504 / [#499](https://github.com/ymnao/scripta/issues/499) で 4 経路とも `rejectEndSymlinkWhenEmulated` を前置し、拒否を `O_EXCL` / `mkdir` の platform 依存な挙動に委ねないようにした（`fs:create-directory` の排他性だけは `O_EXCL` ではなく非 recursive `mkdir` の EEXIST に由来する）（win32 でだけ既存 symlink 上の create が `Already exists` ではなく ELOOP になる）
...
- ~~**retarget 直後は両者が一時的にズレる**~~ → **[#453](https://github.com/ymnao/scripta/issues/453) で解消**。本 ADR の時点では、scan 側の `resolveInsideRoot` が毎回 fresh に解決する（#406 Finding 1）のに対し、fs:read 側の `assertPathAllowed` は **LRU 256 件の `realpathCache`** を使い retarget への invalidation を持たなかったため、symlink を張り替えた直後にその path が cache に載っていると「scan は新しい解決先で判定し、fs:read は古い判定を返す」窓が開いた（本 ADR の不変条件は fs:read 側の cache が fresh な範囲でしか成立していなかった）。#453 で **user-IPC 認可の realpath cache を撤去**し、`assertPathAllowed` / `assertWritePathAllowed` / `isPathAllowed` / `isPathWithinAnyAllowedRoot` も毎回 fresh に解決するようにしたため、**不変条件が cache の温度という前提条件を失った**（本 bullet が受容していたズレは解消。ただし本節の他 bullet が挙げる例外 — L2 hit 経路 / Windows / FileTree 非対称 — は引き続き残る）。判断の根拠は「cache key が成功した path 自身なので鮮度を保存するのは同一 path の 2 回目以降だけだった」「invalidation の通知経路が無いのは `resolveInsideRoot` が cache を通さない理由と同根」「削減できていたのは認可 1 回あたり realpath 1 回 = 実測 ~22µs」の 3 点（詳細は `path-guard.ts` の `realpathBestEffort` doc）
...
- ~~**swap 窓では検索が fs:read より厳しくなる**~~ → **[#418](https://github.com/ymnao/scripta/issues/418) で解消**。本 ADR の時点では `fs:read`（`readFileImpl`）が canonical を plain open していたため、末端を swap された瞬間に「検索結果から落ちるが fs:read は開ける」状態になり得た（落とす側に倒れるので境界は破れない非対称）。#418 で `fs:read` / `fs:read-base64` / `fs:write` も O_NOFOLLOW 付き fd I/O に揃え、**末端 swap 後の判定材料が scan 側と同じ「fresh な realpath 1 回」になった**（in-root なら解決先を読む / 外なら拒否）。当時は cache stale と真の swap を切り分けるため ELOOP を受けたら cache を捨てて再認可していたが、#453 で cache 自体を撤去したので認可が最初から fresh になり、ELOOP は「realpath が解決できない symlink（dangling / 循環）」か「認可から open の間の真の race」だけを意味する fail-closed なシグナルに純化した（後者のうち解決先が workspace 内のものは再試行で通りうるが、窓が handler 1 実行内の await 1 回分まで縮んだため専用の再試行機構は持たない）。残る差は 2 つ: ① scan の分岐 1（index ゲート評価済みかつ indexable）は ELOOP を解決し直さずその pass を skip するため、swap が起きた瞬間の 1 pass だけ検索結果から落ちる（in-root への swap なら次の pass で解決先から読まれて戻る。workspace 外への swap は次 pass 以降も落ち続けるが、それは fs:read の拒否と一致している） ② ~~win32 では O_NOFOLLOW が効かないので両者とも従来挙動~~ → **#451 で解消**（`lstat` エミュレーションで拒否は全 platform で成立し、残る差は「open と原子的か / 別 syscall か」だけになった。上の Windows bullet を参照）
...
- 実装: `electron/main/ipc/search.ts`（`processMdFilesParallel`）/ `electron/main/utils/open-nofollow.ts`（#418 で `read-nofollow.ts` から改名）/ `electron/main/utils/path-guard.ts`（`resolveInsideRoot` / `assertPathAllowed`）
