...
- ~~**swap 窓では検索が fs:read より厳しくなる**~~ → **[#418](https://github.com/ymnao/scripta/issues/418) で解消**。本 ADR の時点では `fs:read`（`readFileImpl`）が canonical を plain open していたため、末端を swap された瞬間に「検索結果から落ちるが fs:read は開ける」状態になり得た（落とす側に倒れるので境界は破れない非対称）。#418 で `fs:read` / `fs:read-base64` / `fs:write` も O_NOFOLLOW 付き fd I/O に揃え、**末端 swap 後の判定材料が scan 側と同じ「fresh な realpath 1 回」になった**（in-root なら解決先を読む / 外なら拒否）。当時は cache stale と真の swap を切り分けるため ELOOP を受けたら cache を捨てて再認可していたが、#453 で cache 自体を撤去したので認可が最初から fresh になり、ELOOP は「dangling か真の race」だけを意味する fail-closed なシグナルに純化した。残る差は 2 つ: ① scan の分岐 1（index ゲート評価済みかつ indexable）は ELOOP を解決し直さずその pass を skip するため、swap が起きた瞬間の 1 pass だけ検索結果から落ちる（in-root への swap なら次の pass で解決先から読まれて戻る。workspace 外への swap は次 pass 以降も落ち続けるが、それは fs:read の拒否と一致している） ② win32 では O_NOFOLLOW が効かないので両者とも従来挙動（#451）
