...
- **実 Electron で書く**: mock では踏めない main 境界に限定する。1 spec = 1「領域」（main 境界）に対応させ、`smoke` を土台に以下を safety net 化する:
  ...
  | PDF 改ページ | `pdf-pagebreak.electron.spec.ts` | PDF 改ページ hybrid アプローチ (#93) |
  | 構造化エラー | `structured-error.electron.spec.ts` | 構造化 IPC エラーの preload round-trip (`getErrorKind`) |
...
- `electron-vite@5` が Electron 42+ に対応した後、build target workaround と合わせて launch helper を見直す。
