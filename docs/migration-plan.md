...
- [ ] `electron/main/utils/search-pure.ts` に `buildLowerToOrigUtf16Map` を実装
      （renderer 側 `highlight-query.ts` のコピー、keep-in-sync コメントで管理）
- [ ] `electron/main/ipc/search.ts` に `searchFilesImpl` を実装
      （UTF-16 ベースで `indexOf` ループ、caseSensitive デフォルト false）
- [ ] `search:files` IPC ハンドラを no-op から実呼び出しに置換
- [ ] 旧 Rust テスト（İ / 絵文字 / 多バイト / 複数マッチ等）を Vitest に移植
