...
- [ ] `searchFilenames` を `fs/promises` 再帰走査で実装（`fast-glob` は不採用、依存を増やさず標準ライブラリで完結。Stage 3 で全文検索も同方針の純 JS に統合）
- [ ] `scan_unresolved_wikilinks` を Node.js で再実装（旧 Rust 実装ロジックを移植）
- [ ] ワークスペース永続化: 自前 settings IPC（`app.getPath("userData") + "/settings.json"`）で前回開いていたフォルダパスを保存・復元（`electron-store` は不採用、`docs/parity-checklist.md` § 7 参照）
...
- [ ] OGP: 共通 HTTP ラッパー（`electron/main/utils/http-fetch.ts`、Node 標準 `http(s).request` ベース）でフェッチ → 自前 OGP パーサ（`electron/main/utils/ogp-parser.ts`、旧 Rust `parse_ogp` を 1:1 移植）でパース → `OgpData` に整形。SSRF 防御（プライベート IP / ループバック / リンクローカルのブロック + DNS rebinding 対策の `pinSafeLookup`）を main 側で行う。`undici` / `cheerio` は不採用（依存最小化、`docs/specification.md` § 2 Node 側ライブラリ方針参照）
...
- [ ] アップデートチェック: GitHub Releases API ポーリングで latest tag を取得し、新しいバージョンがあれば UI で通知（旧 Tauri 版同等の存在確認のみ運用、auto-download / auto-install は採用せず）
