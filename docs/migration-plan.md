...
- ~~コードサイニング / 公証（macOS notarization、Windows EV 証明書）パイプラインを構築~~ — **採用せず**（旧 Tauri 版同等の方針、未署名で出荷）。詳細は `docs/parity-checklist.md` § 11
- [ ] CI 整備: lint / typecheck / test / e2e / ビルド成果物アーティファクト化
- [ ] **機能パリティ確認** チェックリストを通し、旧版と同等以上であることを確認
- [ ] リリース切り替え: 旧 GitHub repo `ymnao/scripta` を `ymnao/scripta-tauri` 等にリネーム → 本リポジトリを `ymnao/scripta` として公開（旧版ユーザーへの移行案内をリリースノートで実施）
- ~~`electron-updater` で自動ダウンロード / 自動インストールの自動アップデート配信~~ — **採用せず**（GitHub Releases API ポーリングによる存在確認のみで運用）。詳細は `docs/parity-checklist.md` § 6
