...
| レイヤー | 技術 | バージョン | 選定理由 |
|---------|------|-----------|---------|
| デスクトップフレームワーク | Electron | v43 | Chromium 固定で挙動が一貫。エコシステムが成熟。`webContents.printToPDF` 等の機能が強力 |
...
| e2e | Playwright | latest | renderer-only モード（Vite dev server + `window.api` モック注入）。実 Electron 起動の `_electron` API ベース e2e は v1.0.0 昇格条件として追加予定 |
| パッケージング | electron-builder | latest | macOS / Windows / Linux のマルチプラットフォーム配布 |
| アップデートチェック | 自前 IPC + 共通 HTTP ラッパー（`electron/main/utils/http-fetch.ts`） | — | GitHub Releases API ポーリングで latest tag を取得し UI で通知（存在確認のみ、auto-download / auto-install は採用せず）。`electron-updater` は導入しない |
| パッケージマネージャ | pnpm | v11+ | 高速インストール、厳密な依存解決。設定は `pnpm-workspace.yaml` |
...
| ワークスペース走査 | `fs/promises` 再帰走査（`electron/main/ipc/search.ts:walkMdFiles` 等） | 旧 Tauri 版 Rust の `collect_md_files` を 1:1 移植。`.` 始まりのエントリ skip / シンボリックリンク扱い等の挙動も合わせる。`fast-glob` は不採用（依存を増やさず標準ライブラリで完結） |
| ファイル変更監視 | `chokidar` | クロスプラットフォーム対応 |
| 全文検索 | 純 JS 実装（`electron/main/utils/search-pure.ts`） | 旧 Tauri 版の Rust ロジック（`String::find` + `to_lowercase` UTF-16 ループ）を 1:1 移植。ripgrep sidecar は不採用（`docs/migration-plan.md` Stage 3 参照） |
| Git 操作 | `simple-git` | git CLI ラッパー |
| HTTP フェッチ | 自前 HTTP ラッパー（`electron/main/utils/http-fetch.ts`） | Node 標準 `http(s).request` を Promise + timeout + max-body + SSRF-safe lookup（`pinSafeLookup`）でラップ。OGP fetch / アップデートチェック双方で共通利用。`undici` は不採用（標準 API で要件を満たせる + 依存最小化） |
| OGP メタ抽出 | 自前パーサ（`electron/main/utils/ogp-parser.ts`） | `<head>` 内の OGP / `<title>` を線形 scan で抽出（旧 Rust `parse_ogp` 1:1 移植）。title / description / image / site_name の 4 タグに限定で十分なため `cheerio` / `parse5` 等の DOM パーサは依存サイズで不採用 |
| ゴミ箱移動 | `electron.shell.trashItem` | OS ネイティブのゴミ箱と連携 |
| 永続化（設定） | 自前 IPC（`fs/promises` で JSON ファイル I/O） | `app.getPath("userData") + "/settings.json"` に保存。ワークスペースパス・ウィンドウ位置等。`electron-store` は不採用（`docs/parity-checklist.md` § 7 参照） |
...
| WS-2 | 最後のワークスペースを記憶 | アプリ再起動時に前回のワークスペースを自動で開く（`settings.json` で永続化、§ 2 ノード側ライブラリ方針参照） |
...
| SR-3 | ワークスペース横断検索 | `Cmd+Shift+F` でサイドバーを検索パネルに切り替え、全ファイルを横断検索（純 JS、旧 Rust ロジック 1:1 移植）。結果をファイル別にグルーピング表示し、クリックで該当行にジャンプ |
| SR-4 | コマンドパレット | `Cmd+P` でファイル名によるファジー検索・ジャンプ |
| SR-5 | 未解決リンク一覧 | `Cmd+Shift+U` でワークスペース内の未解決 Wikilink を一覧表示 |

**本文検索の対象境界**（SR-3 と、同じ scan 基盤を使う未解決 Wikilink 一覧 / バックリンク収集）: ワークスペース内に張られた symlink は、解決先がワークスペース内なら解決先の内容を検索し、ワークスペース外を指すものは検索対象外とする。これは本文検索の結果に出るファイル集合を `fs:read` で開けるファイル集合に揃えるため（開けないファイルの本文を結果に出さない）。ファイルを読まない SR-4（ファイル名検索）はこの境界の対象外で、外部を指す symlink も候補に残る。詳細と受容した例外は [ADR-0011](adr/0011-search-visibility-follows-fs-read.md) を参照。
...
| ID | 機能 | 説明 |
|----|------|------|
| UP-1 | 自動チェック | GitHub Releases API ポーリングで latest tag を取得し、新しいバージョンがあれば UI で通知（存在確認のみ、auto-download / auto-install は採用せず）。詳細は `docs/parity-checklist.md` § 6 |
...
│   │       ├── search.ts           # 全文検索（純 JS、旧 Rust 1:1 移植）・ファイル名検索
