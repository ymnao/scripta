...
**検索対象の境界**: ワークスペース内に張られた symlink は、解決先がワークスペース内なら解決先の内容を検索し、ワークスペース外を指すものは検索対象外とする。これは検索結果に出るファイル集合を `fs:read` で開けるファイル集合に揃えるため（開けないファイルの本文を結果に出さない）。詳細と受容した例外は [ADR-0011](adr/0011-search-visibility-follows-fs-read.md) を参照。
