...
**検索対象の境界**: 検索結果に出るファイル集合は `fs:read` で開けるファイル集合と一致する（[ADR-0011](adr/0011-search-visibility-follows-fs-read.md)）。ワークスペース内に張られた symlink は、解決先がワークスペース内なら解決先の内容を検索するが、ワークスペース外を指すものは検索対象外（開けないファイルの本文を結果に出さない）。
