...
> Stage 6 完了判定の一部。本リポジトリ（Electron 版）が旧 `ymnao/scripta`（Tauri 版、`~/development/tools/scripta`）と同等以上であることを、リリース切り替え前に検証する。
>
...
- 旧 Tauri 版コマンド一覧: `~/development/tools/scripta/src-tauri/src/lib.rs` の `invoke_handler!` ブロック（`commands::*` 31 個 + ルート定義の `clear_webview_browsing_data` で計 32）
...
- [x] § 10 の Playwright `_electron` API ベース e2e 追加（Phase 1 PR-3, #33）。renderer-only の限界（実 IPC / contextBridge / preload / main 境界）を補う 5 領域 safety net を `e2e/electron/` に実装。設定永続化テストは「seed → 復元」「UI 操作 → ディスク永続化 → 再起動復元」の再起動跨ぎ往復を含む。
...
- [x] [#33](https://github.com/ymnao/scripta-next/issues/33) test: Playwright `_electron` API ベースの最小 e2e を追加（Phase 1 PR-3 で実装。#82 C の safety net と統合）
