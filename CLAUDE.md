...
>
> 旧リポジトリ（参照専用、ローカル）: `/Users/nakiym/development/tools/scripta`
> 機能的なリファレンス・既存実装の参照元として活用する。新規実装は本リポジトリ側に行う。
...
**pnpm 11** + **Node.js 22 系**。具体的な version は `package.json` の `packageManager` と `engines.node` を正準として参照する（patch 版までこのドキュメントに書くと同期漏れの温床になるため、本セクションには major 表記のみ残している）。
...
electron 42.0.0 のリリースに伴って 3 種類の workaround が必要になった。いずれも `electron-vite@5.0.0` が 42 に未対応であることに起因する過渡期の対応（1・2 は [alex8088/electron-vite#904](https://github.com/alex8088/electron-vite/issues/904) が landed したら削除予定、3 は electron-vite の electron→node target マップが 42+ に更新されたら見直し）。
...
`electron-vite@5.0.0` の `getElectronNodeTarget()` / `getElectronChromeTarget()` は Electron 39 までしか electron→node / electron→chromium のマップを持たず、42 では `node16.17` / `chrome108` へ stale fallback する（過度な down-level transpile）。`electron.vite.config.ts` で main / preload に `target: "node24"`（Electron 42 同梱 Node v24.15）、renderer に `target: "chrome148"`（Electron 42 = Chromium 148、`electron-to-chromium` の 42.0→148 マッピングに基づく）を明示してこれを矯正している。`engines.node`（npm script を回すホスト Node 条件）は Electron 同梱の runtime Node 版（v24.15）とは別物である点に注意。

renderer は `esnext` ではなく**実 Chromium 版**（`chrome148`）を指す点が重要。`esnext` は「esbuild が最新扱いする構文を極力変換しない」指定なので、将来構文が依存や自前コードに混入すると build は通っても Electron 42 の実行時に parse error になり得る。固定 Chromium に対応するバージョンを明示することで、その範囲へ確実に down-level させる。

`NODE_TARGET` / `RENDERER_TARGET` は `package.json` の electron version と独立してメンテされるため、**Electron をアップグレードする際は** electron-vite が新バージョンの Node / Chromium を内部マップに追加済みかを確認し、未対応なら両定数を新 Electron 同梱の Node / Chromium（`electron-to-chromium` で確認）に追従させる。
