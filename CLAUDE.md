...
`electron-vite@5.0.0` の `getElectronNodeTarget()` は Electron 39 までしか electron→node のマップを持たず、42 では `node16.17` へ stale fallback する（過度な down-level transpile）。`electron.vite.config.ts` で main / preload に `target: "node22"`（Electron 42 同梱 Node `>= 22.12`）、renderer に `target: "esnext"`（Chromium 134+）を明示してこれを矯正している。

`NODE_TARGET` は `package.json` の electron version と独立してメンテされるため、**Electron をアップグレードする際は** electron-vite が新バージョンの Node range をマップに追加済みかを確認し、未対応なら `NODE_TARGET` を新 Electron 同梱 Node に追従させる（renderer の `esnext` は Chromium 固定なので追従不要）。
