...
// **Windows の末端側は経路によって水準が違う**: `O_NOFOLLOW` が無く flag が 0 に落ちるため、
// `open-nofollow.ts` の helper を通る経路 (index 取り込み / scan の read、fs:write、
// git:resolve-conflict) だけが open の前に `lstat` を挟んで拒否をエミュレートしている
// (#451)。そこでも lstat と open の間の差し替え race は残る (POSIX は同じ拒否を open と
// 原子的に行うのでこの窓を持たない)。**helper を経由しない fs:read / fs:read-base64
// (`NOFOLLOW_READ_FLAGS` の直接利用) と pdf:export (`rename(2)` に乗る) は対象外**で、
// win32 では末端 swap を検出しない。経路ごとの根拠は utils/open-nofollow.ts の doc を参照。
